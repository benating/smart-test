# Infrastructure for routing requests to lambda (api gateway)

resource "aws_api_gateway_rest_api" "api" {
  name = "smart-test-http-api"
}
...
resource "aws_api_gateway_integration" "apigw-integration" {
  ...
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.http-lambda-function.invoke_arn
}
...
# Allow cloudwatch logging from API gateway
...
resource "aws_iam_policy" "apigw-logging" {
  name        = "smart-test-apigw-logging-policy"
  path        = "/"
